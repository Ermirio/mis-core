...
module.exports = {
    ...
    /**
     * MIS Core — adminAuth gerenciado pelo Django.
     *
     * Em vez de manter `users:[...]` aqui (exigindo `node-red-admin hash-pw`
     * + edição manual deste arquivo + restart do container para cada novo
     * usuário), validamos as credenciais via HTTP contra o Django, onde o
     * superusuário cria/edita os "Usuários Node-RED" pelo admin
     * (`/mis-core-admin/equipamentos/noderuser/`).
     *
     * Endpoints consumidos:
     *   POST /api/auth/nodered/authenticate/  {username, password}
     *        → 200 {username, permissions}  |  401
     *   POST /api/auth/nodered/user/          {username}
     *        → 200 {username, permissions}  |  404
     *
     * Variáveis de ambiente (definidas em docker-compose):
     *   DJANGO_HOST   default: "django"
     *   DJANGO_PORT   default: 8000
     *
     * Permissões aceitas pelo Node-RED:
     *   "*"     → admin total
    ...
    adminAuth: {
        ...
        users: function (username) {
            return new Promise(function (resolve) {
                ...
                const req = http.request({
                    hostname: process.env.DJANGO_HOST || 'django',
                    port: process.env.DJANGO_PORT || 8000,
                    path: '/api/auth/nodered/user/',
                    method: 'POST',
                    headers: {
        ...
        authenticate: function (username, password) {
            return new Promise(function (resolve) {
                ...
                const req = http.request({
                    hostname: process.env.DJANGO_HOST || 'django',
                    port: process.env.DJANGO_PORT || 8000,
                    path: '/api/auth/nodered/authenticate/',
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Content-Length': Buffer.byteLength(data),
        ...
        // Acesso sem login fica negado — usuário precisa ter conta em
        // /api/auth/nodered/user/ para entrar. `default` precisa retornar
        // uma Promise em Node-RED >= 3.x (sem isso, comms.js quebra
    ...
    */
    ...
    logging: {
        ...
        console: {
             ...
             */
            // MIS Core: nível elevado para diagnosticar problemas de
            // comunicação OPC/MQTT e estouros de memória em flows pesados.
            // Em produção estável, reduza para "info".
            level: "debug",
            /** Whether or not to include metric events in the log output */
            metrics: false,
            /** Whether or not to include audit events in the log output */
            // MIS Core: audit habilitado para rastrear quem (qual usuário
            // Django→Node-RED) fez cada deploy/alteração.
            audit: true
        }
    },
     ...
     */
    // MIS Core: persistência em /data/context para que variáveis de flow e
    // global sobrevivam a reinicializações do container. Útil para guardar
    // contadores acumulados, último valor visto de tag, estado de máquinas
    // de estados em flows, etc. Sem isso, qualquer `docker compose restart
    // node-red` zera tudo.
    contextStorage: {
        default: {
            ...
            config: {
                dir: "/data/context",
                flushInterval: 30,    // segundos entre flushes para disco
                cache: true,          // mantém cópia em memória para leitura rápida
            },
        },
    },
    ...
    editorTheme: {
        ...
        projects: {
            // MIS Core: Projects feature ativado para versionamento Git
            // NATIVO do Node-RED (repositório local em /data/projects).
            // Convive com o NodeRedSnapshot do Django:
            //   - Projects → histórico fino (commit por mudança, branches,
            //     diff visual no editor, rollback rápido sem sair do Node-RED).
            //   - NodeRedSnapshot → snapshots agendados/manuais expostos no
            //     admin Django (auditoria centralizada, restore controlado
            //     por permissão Django).
            enabled: true,
            workflow: {
                // "manual" exige clique em Commit no editor — preferido em
                // produção para evitar histórico ruidoso. Cada operador
                // pode mudar para "auto" em User Settings se preferir.
                mode: "manual"
            }
        },
