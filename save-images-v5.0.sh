...
# ============================================================
# save-images-v5.0.sh
# Salva as imagens atualizadas (flask v5.0 + frontend v5.0)
# para implantação no servidor OT offline.
#
# Alterações incluídas nesta versão:
#   - mis-core-flask:5.0  → Suporte de Backend Analítico a Variáveis Discretas e n_unique heurístico
#   - mis-core-frontend:5.0 → Renderização de Gráfico SPC Bar, Trend Step, Correlation Alert e Stats com Categoria
# ============================================================
set -e

OUTDIR="dist/deploy"
OUTFILE="$OUTDIR/mis-core-images-v5.0.tar"

mkdir -p "$OUTDIR"

echo "================================================="
echo " SALVANDO IMAGENS v5.0 para servidor OT offline"
echo "================================================="

# Apenas as 2 imagens atualizadas nesta versão
UPDATED_IMAGES=(
  "mis-core-flask:5.0"
  "mis-core-frontend:5.0"
)
...
for img in "${UPDATED_IMAGES[@]}"; do
  if docker image inspect "$img" &>/dev/null; then
    ...
    echo "         docker compose build frontend flask"
    exit 1
  fi
done
...
echo "    docker load -i mis-core-images-v5.0.tar"
echo ""
