...
class ConfigManager:
    ...
    def get_config(self, codigo, slug=None):
        if time.time() - self.last_update > self.CACHE_DURATION:
            self._atualizar_cache()
        return self.equipamentos.get(slug) or self.equipamentos.get(codigo, {'vel_nominal': 100})
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for eq in lista:
                    ...
                    if eq.get('slug'):
                        novas_configs[eq['slug']] = cfg
                    # Legacy callers still pass codigo. Slug callers avoid the
                    # collision when E001 exists in more than one line.
                    novas_configs.setdefault(eq['codigo'], cfg)
...
class ProductionEngine:
    ...
    def _load_state_from_db(
        self, state_key, current_op, current_shift,
        equipment_code=None, linha_codigo=None, equipamento_slug=None,
    ):
        state = self._get_state(state_key)
        if state['initialized']: return

        try:
            if equipamento_slug and equipment_code and linha_codigo:
                equipment_where = (
                    f"(\"equipment_slug\" = '{equipamento_slug}' "
                    f"OR (\"equipment\" = '{equipment_code}' AND \"line\" = '{linha_codigo}'))"
                )
            elif equipamento_slug:
                equipment_where = f"\"equipment_slug\" = '{equipamento_slug}'"
            elif equipment_code and linha_codigo:
                equipment_where = f"\"equipment\" = '{equipment_code}' AND \"line\" = '{linha_codigo}'"
            else:
                equipment_where = f"\"equipment\" = '{equipment_code or state_key}'"

            query = f"SELECT * FROM production WHERE {equipment_where} ORDER BY time DESC LIMIT 1"
            ...
            if points:
                ...
                if last_shift == curr_shift:
                    ...
                    state['acc_waste_shift'] = int(
                        d.get('refugo_turno_acumulado')
                        or d.get('descarte_turno_acumulado')
                        or d.get('descarte')
                        or 0
                    )
    ...
    def processar_dados(
        self, equipamento, op_atual, contagem_bruta, descarte,
        formato_gramas, planejado, velocidade_atual, estado_maquina,
        extra_context=None, contagem_entrada=0, equipamento_slug=None,
        linha_codigo=None,
    ):
        # 1. Contexto
        ...
        state_key = equipamento_slug or (
            f"{linha_codigo}.{equipamento}" if linha_codigo else equipamento
        )
        config = self.config_manager.get_config(equipamento, equipamento_slug)
        vel_nominal = config['vel_nominal']

        # Atualiza Heartbeat
        self.heartbeats[state_key] = now_timestamp

        # 2. Inicializa memória
        self._load_state_from_db(
            state_key, op_atual, turno_atual,
            equipment_code=equipamento,
            linha_codigo=linha_codigo,
            equipamento_slug=equipamento_slug,
        )
        state = self._get_state(state_key)
        state.setdefault('acc_waste_shift', 0)
        ...
        # Produção de turno: o CLP é a fonte de verdade.
        # A contagem do CLP deve zerar na virada de turno; portanto, durante
        # um turno válido, o valor atual do contador já representa a produção
        # atual do turno. Evita carregar produção de OP/turno anterior no card.
        descarte_por_balanco = 0
        if contagem_entrada and contagem_entrada > 0:
            descarte_por_balanco = max(0, int(contagem_entrada or 0) - int(contagem_bruta or 0))

        state['acc_shift'] = max(0, int(contagem_bruta or 0))
        state['acc_waste_shift'] = max(0, int(descarte or 0), descarte_por_balanco)
        ...
        if tempo_planejado <= 0 or tempo_produtivo <= 0:
            ...
            performance_raw = (
                float(velocidade_atual or 0) / float(vel_nominal)
                if vel_nominal and vel_nominal > 0 and estado_maquina == 1
                else 0.0
            )
            performance = min(1.0, performance_raw)
            quality_total = (state['acc_shift'] + state['acc_waste_shift']) or total_input_op
            quality_good = state['acc_shift'] or total_prod_op
            quality_valid = quality_total > 0
            qualidade = (quality_good / quality_total) if quality_valid else 1.0
            oee_valid = quality_valid and estado_maquina == 1
        ...
        total_waste_turno = state['acc_waste_shift']
        ton_turno = (total_turno * formato_gramas) / 1_000_000.0 if formato_gramas else 0
        ton_refugo_turno = (total_waste_turno * formato_gramas) / 1_000_000.0 if formato_gramas else 0
        total_count_turno = total_turno + total_waste_turno
        percentual_descarte_turno = (total_waste_turno / total_count_turno * 100.0) if total_count_turno > 0 else 0.0
        ...
        metrics = {
            ...
            'refugo_turno': int(total_waste_turno),
            'descarte_turno': int(total_waste_turno),
            'refugo_turno_acumulado': int(total_waste_turno),
            'toneladas_turno': float(ton_turno),
            'toneladas_refugo_turno': float(ton_refugo_turno),
            'percentual_descarte_turno': float(percentual_descarte_turno),
