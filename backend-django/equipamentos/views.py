...
class EquipamentoViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        linha_codigo = self.request.query_params.get('linha__codigo') or self.request.query_params.get('linha_codigo')
        if linha:
            if str(linha).isdigit():
                queryset = queryset.filter(linha_id=linha)
            else:
                queryset = queryset.filter(linha__codigo=linha)
        if linha_codigo:
            queryset = queryset.filter(linha__codigo=linha_codigo)
        return queryset
    ...
    def sync_metadata(self, request):
        ...
        try:
            ...
            # Pós PR 3: codigo nao e mais unico global - pode haver E001 em
            # L01 e em L02. Aceitamos 'linha_codigo' opcional para desambiguar.
            linha_codigo = data.get('linha_codigo') or data.get('linha')
            qs = Equipamento.objects.filter(codigo=equipamento_codigo)
            if linha_codigo:
                qs = qs.filter(linha__codigo=linha_codigo)
            equipamento = qs.first()
            if not equipamento:
                return Response({'error': f'Equipamento {equipamento_codigo} não encontrado'}, status=status.HTTP_404_NOT_FOUND)
            if qs.count() > 1:
                return Response({
                    'error': f'Equipamento {equipamento_codigo} existe em múltiplas linhas — passe linha_codigo para desambiguar.'
...
def _incluir_inativos(request) -> bool:
    """True se o caller pediu explicitamente ?incluir_inativos=true (ou 1)."""
    raw = (request.query_params.get('incluir_inativos') or '').strip().lower()
    return raw in ('1', 'true', 'yes', 'sim')


class TagColetaViewSet(viewsets.ModelViewSet):
    queryset = TagColeta.objects.select_related('equipamento', 'equipamento__linha')
    serializer_class = TagColetaSerializer

    def get_queryset(self):
        qs = super().get_queryset()
        if not _incluir_inativos(self.request):
...
class SensorViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'])
    def por_linha(self, request):
        """Retorna sensores de uma linha específica (respeita ?incluir_inativos)."""
        linha_id = request.query_params.get('linha_id')
        if linha_id:
            sensores = self.get_queryset().filter(linha_id=linha_id)
...
def eventos_estado(request):
    ...
    try:
        ...
        # Busca o equipamento pelo código (pós PR 3: codigo não é mais unico
        # global; aceita linha_codigo opcional para desambiguar).
        linha_codigo = data.get('linha_codigo') or data.get('linha')
        qs = Equipamento.objects.filter(codigo=data['equipamento_codigo'])
        if linha_codigo:
            qs = qs.filter(linha__codigo=linha_codigo)
        equipamento = qs.first()
        if not equipamento:
        ...
        if qs.count() > 1:
            return Response({
                'status': 'error',
                'message': f"Equipamento '{data['equipamento_codigo']}' existe em múltiplas linhas — passe linha_codigo."
            }, status=status.HTTP_409_CONFLICT)
        ...
        evento_atual = eventos_abertos[0] if eventos_abertos else None
        if evento_atual and evento_atual.estado == data['estado']:
            for duplicado in eventos_abertos[1:]:
                duplicado.fim = max(duplicado.inicio, evento_atual.inicio)
                duplicado.save(update_fields=['fim', 'duracao_segundos'])
            return Response({
                'status': 'success',
                'message': 'Estado já estava ativo; evento reutilizado',
                'evento_id': evento_atual.id,
                'evento_anterior_fechado': None,
            }, status=status.HTTP_200_OK)

        evento_fechado = None
        for aberto in eventos_abertos:
            aberto.fim = max(aberto.inicio, data['timestamp'])
            aberto.save(update_fields=['fim', 'duracao_segundos'])
            if evento_fechado is None:
                evento_fechado = aberto
