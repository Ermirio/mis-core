    ...
    def consolidar_horario(self, client, horas_atras=1):
        ...
        try:
            ...
            for (key, points) in result.items():
                ...
                # Buscar objetos Django — desambigua sempre por (linha, codigo)
                # ou por slug se o equipment_code já veio nesse formato.
                try:
                    linha = LinhaProducao.objects.get(codigo=line_code)
                    if equipment_code and '.' in equipment_code:
                        equipamento = Equipamento.objects.filter(slug=equipment_code).first()
                    else:
                        equipamento = Equipamento.objects.filter(
                            linha=linha, codigo=equipment_code,
                        ).first()
