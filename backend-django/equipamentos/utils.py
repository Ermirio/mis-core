...
def get_meta_turno(linha, data, turno):
    """Retorna a meta de produção para (linha, data, turno).

    Ordem de resolução:
    1. CalendarioProducao(data, linha, turno).meta_producao_turno, se existir
       E for > 0.
    2. linha.meta_producao_turno (fallback padrão programado da linha).

    Centraliza a regra para não duplicar nos consumidores Django/Flask.
    """
    from .models import CalendarioProducao  # import lazy para evitar ciclo
    try:
        cal = (
            CalendarioProducao.objects
            .filter(linha=linha, turno=turno, data=data)
            .first()
        )
        if cal and cal.meta_producao_turno and cal.meta_producao_turno > 0:
            return cal.meta_producao_turno
    except Exception:
        pass
    return getattr(linha, 'meta_producao_turno', 0) or 0
