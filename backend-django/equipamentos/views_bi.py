...
class OrdemProducaoViewSet(viewsets.ModelViewSet):
    ...
    def auto_create_or_get(self, request):
        ...
        # 2. Se não achou, tentar pelo identificador do equipamento.
        # Aceita slug ("L01.E001") OU código curto ("E001"). Slug é exato;
        # código curto cai no resolver central (que detecta ambiguidade).
        if not linha and codigo_equipamento:
            from equipamentos.models import Equipamento
            from equipamentos.resolvers import resolver_equipamento, EquipamentoAmbiguo
            try:
                if '.' in codigo_equipamento:
                    equipamento = resolver_equipamento(slug=codigo_equipamento)
                else:
                    equipamento = resolver_equipamento(
                        codigo=codigo_equipamento,
                        linha_codigo=codigo_linha,
                    )
                linha = equipamento.linha
            except (Equipamento.DoesNotExist, EquipamentoAmbiguo):
                equipamento = None
