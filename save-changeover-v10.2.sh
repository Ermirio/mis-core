#!/bin/bash
# ============================================================
# save-changeover-v10.2.sh
#
# Patch incremental v10.1 -> v10.2.
#
# O que muda:
#   * Django (mis-backend:v10.2):
...
#
#   * Recipe Monitor (mis-recipe-intelligent:v10.2):
...
#   mis-changeover-v10.2.tar             (2 imagens: backend + recipe)
#   carregar-v10.2.sh                    (deploy automatico)
# ============================================================
set -e
...
TAR_FILE="$OUTDIR/mis-changeover-v10.2.tar"
LOAD_SCRIPT="$OUTDIR/carregar-v10.2.sh"

mkdir -p "$OUTDIR"

echo "================================================="
echo " MIS Changeover v10.2 - Patch incremental (backend + recipe-monitor)"
echo "================================================="

echo ""
echo " [1/4] Build mis-backend:v10.2 ..."
docker build -t mis-backend:v10.2 "$CHANGEOVER_DIR/Backend"

echo ""
echo " [2/4] Build mis-recipe-intelligent:v10.2 ..."
docker build -t mis-recipe-intelligent:v10.2 "$RECIPE_DIR"

echo ""
echo " [3/4] Build mis-frontend:v10.2 (com protecao do botao Sincronizar)..."
docker build \
    --build-arg REACT_APP_RECIPE_MONITOR_URL= \
    -t mis-frontend:v10.2 \
    "$CHANGEOVER_DIR/Frontend"

echo ""
echo " [4/4] Salvando 3 imagens em $TAR_FILE ..."
docker save mis-backend:v10.2 mis-recipe-intelligent:v10.2 mis-frontend:v10.2 -o "$TAR_FILE"
SIZE=$(du -sh "$TAR_FILE" | cut -f1)
echo " [OK] $TAR_FILE ($SIZE)"

# Gerar carregar-v10.2.sh
cat > "$LOAD_SCRIPT" << 'LOAD_EOF'
#!/bin/bash
# carregar-v10.2.sh — Patch incremental v10.1 -> v10.2 (backend + recipe-monitor)
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
HUB_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
TAR_FILE="$SCRIPT_DIR/mis-changeover-v10.2.tar"

echo ""
echo "=== MIS Changeover v10.2 - Deploy Incremental ==="
...
for IMG in mis-backend:v10.2 mis-recipe-intelligent:v10.2 mis-frontend:v10.2; do
    docker image inspect "$IMG" >/dev/null 2>&1 || { echo "ERRO: $IMG nao carregada"; exit 1; }
    echo "    OK: $IMG"
done

# 2. Atualizar tags no docker-compose.yml (substitui v10.1 -> v10.2 apenas das 2 imagens)
...
if [ -f "$TARGET_COMPOSE" ]; then
    ...
    # Aceita qualquer subversao 10.x (v10.0, v10.1, ...) e atualiza para v10.2
    sed -i -E 's|mis-backend:v10\.[0-9]+|mis-backend:v10.2|g'                   "$TARGET_COMPOSE"
    sed -i -E 's|mis-recipe-intelligent:v10\.[0-9]+|mis-recipe-intelligent:v10.2|g' "$TARGET_COMPOSE"
    sed -i -E 's|mis-frontend:v10\.[0-9]+|mis-frontend:v10.2|g'                 "$TARGET_COMPOSE"
    echo "    OK"
else
    echo "AVISO: $TARGET_COMPOSE nao existe. Edite manualmente para usar v10.2."
fi
