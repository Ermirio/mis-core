...
const DiagnosticsPanel: React.FC<DiagnosticsPanelProps> = ({ equipamentoCodigo }) => {
    ...
    const confirmApply = async () => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_DJANGO_API_URL || '/api'}/golden-state/apply`, {
                method: 'POST',
            ...
            if (response.ok && data.status === 'queued') {
                ...
                const pollInterval = setInterval(async () => {
                    try {
                        const res = await fetch(`${import.meta.env.VITE_DJANGO_API_URL || '/api'}/golden-state/status/${batchId}`);
    ...
    const fetchDiagnostics = async () => {
        try {
            let url = `${import.meta.env.VITE_DJANGO_API_URL || '/api'}/diagnostics/alerts/${equipamentoCodigo}`;
            if (filterCurrentSku) {
    ...
    const captureGoldenState = async () => {
        setCapturing(true);
        try {
            const response = await fetch(`${import.meta.env.VITE_DJANGO_API_URL || '/api'}/diagnostics/capture/${equipamentoCodigo}`, {
                method: 'POST'
            });
            const data = await response.json();
            if (data.status === 'success') {
