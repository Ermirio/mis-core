...
export function applyDelta(series) {
    const out = [];
    let prev = null;
...
export function maskOff(series) {
    return series.map(p => p.off === 1 ? { ...p, value: null } : p);
}
export function describe(values) {
    const v = values.filter((x) => typeof x === "number" && Number.isFinite(x));
    if (!v.length)
        return { n: 0, mean: null, std: null, min: null, max: null, p50: null, p90: null, p99: null };
...
export function histogram(values, bins = 14) {
    const v = values.filter(x => Number.isFinite(x));
    if (!v.length)
        return { labels: [], counts: [] };
    const min = Math.min(...v), max = Math.max(...v);
    if (min === max)
        return { labels: [min.toFixed(2)], counts: [v.length] };
    const step = (max - min) / bins;
    const labels = [];
    const counts = new Array(bins).fill(0);
    for (let b = 0; b < bins; b++)
        labels.push((min + b * step).toFixed(1));
    v.forEach(x => {
...
export function boxStats(values) {
    ...
    if (!v.length)
        return { q1: 0, q2: 0, q3: 0, min: 0, max: 0, n: 0 };
