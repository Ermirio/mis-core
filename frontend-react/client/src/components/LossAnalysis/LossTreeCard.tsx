      ...
      <div style={{ padding: '16px' }}>
        ...
        {data?.data_quality && (
          <div style={{ marginTop: 12, padding: '8px 10px', borderRadius: 5,
            ...
            Referência: {data.data_quality.reference_equipment || 'não configurada'}
            {' · '}
            {data.data_quality.reference_online ? 'comunicação atual' : 'sem comunicação atual'}
            {data.data_quality.no_production_in_period ? ' · sem produção no período' : ''}
            {data.data_quality.unclassified_minutes > 0
              ? ` · ${data.data_quality.unclassified_minutes.toFixed(1)} min sem dados classificados`
              : ''}
          </div>
        )}
