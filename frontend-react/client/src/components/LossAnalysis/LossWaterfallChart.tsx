...
const ISA = {
  ...
  unknown:'#cbd1d8',   // telemetry unavailable / not classified
} as const;

const CHART_COLORS = [ISA.off, ISA.accent, ISA.bad, ISA.warn, ISA.qual, ISA.ok, ISA.unknown];
...
const LossWaterfallChart: React.FC<LossWaterfallProps> = ({ data, isLoading }) => {
  ...
  const chartData = [
    ...
    { name: 'Sem dados',    value: parseFloat((data.unclassified_time || 0).toFixed(1)), color: CHART_COLORS[6] },
  ];
