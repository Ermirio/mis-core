...
const LineManagement: React.FC = () => {
  ...
  const fetchLinhaStatus = async () => {
    try {
      // Flask-out completo: /linha/<id>/status migrado para Django.
      const response = await fetch(`${DJANGO_API_URL}/linha/${linhaId}/status`);
      if (!response.ok) throw new Error(response.statusText);
      const data = await response.json();
      // Sem fallback para mock — se backend nao tem dados, mostra vazio.
      setLinhaStatus(data?.equipamentos ? data : null);
      setIsDemo(false);
    } catch (err) {
      console.error('LineManagement fetch falhou', err);
      setLinhaStatus(null);
      setIsDemo(false);
    } finally {
      setLoading(false);
