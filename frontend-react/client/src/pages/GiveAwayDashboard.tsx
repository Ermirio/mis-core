...
const GiveAwayDashboard: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // Sem fallback para mock — se backend nao retornar, dashboard fica vazio.
            setData(res.data?.consolidado ? res.data : null);
            setIsDemo(false);
        } catch (err) {
            console.error('GiveAway fetch falhou', err);
            setData(null);
            setIsDemo(false);
        } finally {
            setLoading(false);
    ...
    return (
        <div className="isa-root" style={{ padding: '16px 20px', minHeight: '100vh' }}>
            ...
            {/* Empty state explicito quando nao ha dados de peso_real coletados */}
            {!loading && data && (!data.por_linha || data.por_linha.length === 0) && (
                <Panel title="Como ativar Give Away">
                    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', color: 'var(--isa-text-muted)' }}>
                        <AlertTriangle size={20} style={{ color: 'var(--isa-warn)', flexShrink: 0, marginTop: 2 }} />
                        <div style={{ fontSize: 13, lineHeight: 1.5 }}>
                            <strong style={{ color: 'var(--isa-text)' }}>Sem dados de peso real medido.</strong>
                            <br />Para o cálculo de Give Away funcionar, é preciso:
                            <ol style={{ margin: '6px 0 0', paddingLeft: 18 }}>
                                <li>Cadastrar a tag <code>peso_real</code> (criada automaticamente desde a tranche atual) e ativá-la no admin do equipamento.</li>
                                <li>Configurar o <em>node_id</em> OPC apontando para o checkweigher / enchedora.</li>
                                <li>Opcional: definir <code>formato_alvo_padrao</code> na linha para servir de fallback quando o SKU corrente não tem <code>peso_unitario</code> cadastrado.</li>
                            </ol>
                        </div>
                    </div>
                </Panel>
