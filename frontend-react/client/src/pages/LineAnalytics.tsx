...
import { DJANGO_API_URL as DJANGO_API, FLASK_API_URL as FLASK_API, FASTAPI_V2_URL as FASTAPI_V2 } from '@/config/api';
...
const CorrMatrixPlot = ({
    ...
    // Defesa: o backend pode retornar correlation_matrix vazia ou com
    // colunas/valores nulos quando nao ha pontos suficientes na janela.
    // Antes esse caso quebrava a pagina inteira ("Cannot read properties
    // of null (reading 'length')").
    const columns: string[] | undefined = matrix?.columns;
    const values: number[][] | undefined = matrix?.values;
    const p_values: number[][] | undefined = matrix?.p_values;
    const method: string | undefined = matrix?.method;
            ...
            <Card>
                ...
                <CardContent>
                    <div className="text-center p-8 text-gray-500">
                        Dados insuficientes para calcular a correlação. Verifique se
                        as variáveis selecionadas têm pontos no período escolhido.
                    </div>
                </CardContent>
            </Card>
        );
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription className="text-xs">
                    Clique numa célula para abrir o gráfico de dispersão do par. Células abaixo do corte ficam em branco.
                </CardDescription>
            </CardHeader>
            <CardContent>
                <Plot
...
const LineAnalytics: React.FC = () => {
    ...
    // Re-roda /analyze/correlation quando o método (Pearson/Spearman) muda,
    // sem depender do closure de onClick (que ainda enxerga o método antigo
    // por causa do setState ser assíncrono). Só re-roda se já existe uma
    // matriz visível — o useEffect inicial não dispara em vazio.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => {
        if (!correlationData) return;
        if (selectedTags.length === 0) return;
        handleRunAnalysis('correlation', true);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [corrMethod]);
    ...
    // Overlay de eventos no Trend: bandas verticais (estado_maquina, setup, parada...)
    // permitem "ler o gráfico" — não basta ver a variável caindo, é importante
    // saber se ela caiu PORQUE houve setup, falta de material, manutenção, etc.
    const [showEvents, setShowEvents] = useState(true);
    const [events, setEvents] = useState<Array<{
        id: number; equipamento: number; estado: string;
        inicio: string; fim: string | null;
    }>>([]);

    // Cores ISA-101 por estado para as bandas verticais do overlay.
    // PRODUZINDO é deixado transparente — só desenhamos eventos "não-produzindo".
    const eventColor = (estado: string): string | null => {
        const e = (estado || '').toUpperCase();
        if (e === 'PRODUZINDO' || e === 'RODANDO' || e === '1') return null;
        if (e.includes('SETUP') || e === '5') return 'rgba(192,138,30,';
        if (e.includes('TESTE') || e === '6') return 'rgba(120,80,180,';
        if (e.includes('MANUT') || e === '7' || e === '8') return 'rgba(120,144,156,';
        if (e.includes('FALTA') || e === '9') return 'rgba(181,58,43,';
        if (e.includes('PARTIN') || e === '11') return 'rgba(31,122,59,';
        if (e.includes('PARANDO') || e === '13') return 'rgba(192,138,30,';
        if (e.includes('AGUARD') || e === '2') return 'rgba(120,144,156,';
        if (e.includes('BLOCK') || e === '3') return 'rgba(120,144,156,';
        if (e.includes('PARADO') || e.includes('FALHA') || e === '4') return 'rgba(181,58,43,';
        return 'rgba(120,144,156,';
    };

    // Lê eventos e devolve shapes Plotly (bandas verticais semitransparentes).
    // Cada evento vira um `rect` que vai do y=0 ao y=1 em coordenadas relativas
    // (yref: 'paper'), e em x de inicio a fim em coordenadas reais ('x').
    const eventShapes = useMemo(() => {
        if (!showEvents || events.length === 0) return [];
        const out: any[] = [];
        for (const e of events) {
            const c = eventColor(e.estado);
            if (!c) continue; // PRODUZINDO: sem banda
            const x0 = e.inicio;
            const x1 = e.fim || new Date().toISOString();
            out.push({
                type: 'rect',
                xref: 'x', yref: 'paper',
                x0, x1, y0: 0, y1: 1,
    ...
    // Carrega eventos da linha no range visível, sempre que muda tr ou selectedTags.
    // Restringe aos equipamentos selecionados para não poluir.
    useEffect(() => {
        if (!showEvents || selectedTags.length === 0) {
            setEvents([]);
            return;
        }
        const linhaIds = Array.from(new Set(
            selectedTags.map(t => (t as any).linha_id).filter((x: any) => x != null)
        ));
        ...
        (async () => {
            try {
                const params = new URLSearchParams({
                    linha_id: String(linhaIds[0]),
                    data_inicio: tr.from.toISOString(),
                    data_fim: tr.to.toISOString(),
                    ordering: 'inicio',
                    page_size: '5000',
                });
                const res = await fetch(`${DJANGO_API}/eventos-estado/?${params}`);
                if (!res.ok) return;
                const d = await res.json();
                if (!cancel) setEvents(d.results || []);
            } catch { /* silencioso */ }
        })();
        return () => { cancel = true; };
    }, [showEvents, selectedTags, tr.from, tr.to]);
        ...
        try {
            ...
            const payload: any = {
                variables: selectedTags.map(t => ({
                    ...
                    equipamento_slug: t.equipamento_slug,
                    linha_codigo: t.linha_codigo,
            ...
            try {
                // Flask-out Onda 6: /analyze/* migrado para Django (mesma
                // logica pandas/scipy do Flask). FastAPI v2 nao usado aqui
                // pois schemas sao incompativeis (extra forbid + time_range
                // aninhado) - eliminacao real do Flask passa por Django.
                const res = await axios.post(`${DJANGO_API}${endpoint}`, payload);
                ...
                // Sem fallback de mock. Em modo nao-silencioso, propaga mensagem
                // amigavel para evitar tela branca no consumidor (ex.: CorrMatrixPlot).
                if (!silent) {
                    const status = apiErr?.response?.status;
                    const detail = apiErr?.response?.data?.detail
                        || apiErr?.response?.data?.error
                        || apiErr?.message;
                    // Mensagens especificas que o Flask retorna em /analyze/correlation:
                    //  - "Insufficient overlap data": resample nao tem nenhuma
                    //    linha onde TODAS as vars tem valor (janelas disjuntas)
                    //  - "Insufficient variables with data": apos remover colunas
                    //    vazias, restam < 2 variaveis. Pode ser que a variavel
                    //    selecionada nao existe no Influx (ex.: 'oee' nao e
                    //    escrita pelo coletor, e calculada em runtime).
                    //  - "No data": nenhum ponto no periodo.
                    const detailStr = String(detail || '').toLowerCase();
                    let friendly = `Dados insuficientes (${status}).`;
                    if (detailStr.includes('insufficient overlap')) {
                        friendly = 'As variáveis selecionadas não possuem janelas de tempo em comum. '
                            + 'Tente ampliar o período ou desativar "ignorar OFF".';
                    } else if (detailStr.includes('insufficient variables')) {
                        friendly = 'Apenas uma das variáveis selecionadas tem dados no período. '
                            + 'Correlação exige no mínimo 2 variáveis com leituras. '
                            + 'Observação: métricas calculadas (OEE, descarte, give-away) não são '
                            + 'tags do InfluxDB — selecione tags brutas como velocidade, contagens, peso real.';
                    } else if (detailStr.includes('no data')) {
                        friendly = 'Nenhum dado encontrado para essas variáveis no período. '
                            + 'Confirme se o coletor está escrevendo no Influx e amplie a janela.';
                    } else if (status === 400 || status === 422) {
                        friendly = `Dados insuficientes para a análise (${status}). Detalhe: ${detail || 'sem detalhe'}`;
                    } else if (status >= 500) {
                        friendly = `Erro interno no servidor de análise (${status}). Equipe técnica notificada.`;
                    } else {
                        friendly = `Falha ao executar análise: ${detail || apiErr?.toString()}`;
                    }
                    setError(friendly);
                }
            }
            // Sem fallback para mock: se nao ha dados reais, o componente abaixo
            // mostra "sem dados" naturalmente.
            ...
            if (mode === 'stats') {
                ...
                // Defesa total: responseData pode ser null quando o backend
                // retornou empty/422/erro. Object.keys(null) explode com
                // 'Cannot convert undefined or null to object' e leva a
                // tela branca em /analytics.
                setTimeseriesData(responseData);
                const aliases = responseData && typeof responseData === 'object'
                ...
                if (!silent && !responseData) {
                    setError('Sem dados para a janela selecionada. Tente outro período ou variáveis.');
                }
            }
    ...
    const getEquipmentTags = (linha: Linha, eq: Equipamento) => {
        let tags: any[] = [];
        // Standard Metrics - alinhado com DEFAULT_TAGS_COLETA do Django.
        // ATENCAO: OEE, descarte e giveaway nao sao tags brutas do
        // InfluxDB - sao calculadas pelo agregador. Para analytics
        // (correlation/stats/timeseries), so listamos tags que o
        // coletor realmente escreve no Influx, evitando 400 do Flask
        // "Insufficient data" ao selecionar metricas inexistentes.
        const standardMetrics = [
            { nome: 'Contagem entrada', tag: 'contagem_entrada' },
            { nome: 'Contagem saída', tag: 'contagem_saida' },
            { nome: 'Velocidade', tag: 'velocidade_atual' },
            { nome: 'Peso real medido', tag: 'peso_real' },
            { nome: 'Formato', tag: 'formato' },
            { nome: 'Estado da máquina', tag: 'estado_maquina', isDiscrete: true, valueMapping: ESTADO_VALUE_MAPPING },
        ];
    ...
    // Resumo estatistico sem mutar timeseriesData; Trend/SPC dependem da ordem temporal original.
    const getNumericValues = (values: any[] | undefined): number[] =>
        (values || []).filter((v): v is number => typeof v === 'number' && Number.isFinite(v));

    const getDistributionSummary = (values: any[] | undefined) => {
                ...
                <Card className="an__panel">
                    <CardHeader className="pb-2">
                        ...
                        <CardDescription className="an__panel-desc">
                            ...
                            {/* Aviso discreto: só o ícone com tooltip. Sem
                                poluir o painel lateral. */}
                            <span
                                title={
                                    'OEE, Descarte e Give-Away são métricas calculadas — não aparecem aqui.\n'
                                    + 'Veja nos painéis dedicados:\n'
                                    + ' • Home (OEE)\n • /descartes (refugo)\n • /giveaway (peso real)'
                                }
                                style={{
                                    marginLeft: 6,
                                    ...
                                    cursor: 'help',
                                    borderBottom: '1px dotted var(--isa-text-muted)',
                                }}
                            >
                                ⓘ métricas calculadas
                            </span>
                ...
                <div className="an__main">
                                ...
                                <div className="isa-switches" style={{ marginLeft: 0 }}>
                                    ...
                                    {activeTab === 'trend' && (
                                        <>
                                            ...
                                            <label className="isa-switch" title="Bandas verticais com estados (setup, parada, falta de material...) no gráfico">
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full h-full flex flex-col">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {Object.entries(timeseriesData).map(([alias, d]: [string, any]) => {
                                            const summary = getDistributionSummary(d.values);
                                            return (
                                            <Card key={alias} className="border-l-4 border-l-purple-500">
                                                ...
                                                <CardContent>
                                                    ...
                                                    <div className="flex justify-between text-xs text-gray-500 mt-2">
                                                        <span>Min: {formatSummaryValue(summary.min)}</span>
                                                        <span>Mediana: {formatSummaryValue(summary.median)}</span>
                                                        <span>Max: {formatSummaryValue(summary.max)}</span>
                                                    </div>
                                                </CardContent>
                                            </Card>
                                            );
                        ...
                        <TabsContent value="trend">
                            {timeseriesData ? (
                                <div className="space-y-4">
                                    {/* Legenda compacta dos eventos quando overlay ativo */}
                                    {showEvents && events.length > 0 && (() => {
                                        const tipos = new Map<string, string>();
                                        for (const e of events) {
                                            const c = eventColor(e.estado);
                                            if (!c) continue;
                                            tipos.set(e.estado, `${c}0.35)`);
                                        }
                                        if (tipos.size === 0) return null;
                                        return (
                                            <div style={{
                                                display: 'flex', flexWrap: 'wrap', alignItems: 'center',
                                                gap: 12, fontSize: 11, padding: '6px 10px',
                                                background: 'var(--isa-bg-panel)',
                                                border: '1px solid var(--isa-border)',
                                                borderRadius: 4,
                                            }}>
                                                <strong style={{ color: 'var(--isa-text-muted)' }}>
                                                    Eventos no gráfico:
                                                </strong>
                                                {Array.from(tipos.entries()).map(([nome, cor]) => (
                                                    <span key={nome} style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                                                        <span style={{ width: 12, height: 12, background: cor, borderRadius: 2, display: 'inline-block' }} />
                                                        <span style={{ color: 'var(--isa-text)' }}>{nome}</span>
                                                    </span>
                                                ))}
                                                <span style={{ marginLeft: 'auto', color: 'var(--isa-text-muted)' }}>
                                                    {events.length} evento(s) na janela
                                                </span>
                                            </div>
                                        );
                                    })()}
                        ...
                        <TabsContent value="correlation">
                            {/* Barra de controles: Pearson/Spearman + slider de corte */}
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 16, flexWrap: 'wrap', marginBottom: 10 }}>
                                <div
                                    role="radiogroup"
                                    aria-label="Método de correlação"
                                    style={{
                                        display: 'inline-flex',
                                        border: '1px solid var(--isa-border, #cfd8dc)',
                                        borderRadius: 4,
                                        overflow: 'hidden',
                                        background: 'var(--isa-bg-elev, #fff)',
                                    }}
                                >
                                    {(['pearson', 'spearman'] as const).map(m => {
                                        const active = corrMethod === m;
                                        return (
                                            <button
                                                key={m}
                                                type="button"
                                                ...
                                                title={m === 'pearson'
                                                    ? 'Pearson (r): correlação LINEAR. Bom para sinais contínuos sem outliers.'
                                                    : 'Spearman (ρ): correlação por POSTOS. Robusto a outliers e a relações monotônicas não-lineares.'}
                                                style={{
                                                    padding: '5px 12px',
                                                    fontSize: 12,
                                                    fontWeight: active ? 600 : 500,
                                                    background: active ? 'var(--isa-accent, #1565c0)' : 'transparent',
                                                    color: active ? '#fff' : 'var(--isa-text, #212121)',
                                                    border: 'none',
                                                    cursor: 'pointer',
                                                    borderLeft: m === 'spearman' ? '1px solid var(--isa-border, #cfd8dc)' : 'none',
                                                }}
                                            >
                                ...
                                <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--isa-text-muted)' }}>
                                    <span style={{ whiteSpace: 'nowrap' }}>
                                        Corte |r| ≥ <strong style={{ color: 'var(--isa-text)', minWidth: 28, display: 'inline-block', textAlign: 'right' }}>{corrMinFilter.toFixed(2)}</strong>
                                    </span>
                                    <input
                                        type="range"
                                        min={0}
                                        ...
                                        step={0.05}
                                        value={corrMinFilter}
                                        onChange={e => setCorrMinFilter(Number(e.target.value))}
                                        aria-label="Corte mínimo de correlação"
                                        style={{ width: 140 }}
                                    />
                                </label>
                            </div>
