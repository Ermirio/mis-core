...
class AgregadorDados:
    ...
    def sincronizar_dados_producao(self, equipamento: Equipamento, data_hora: datetime):
        ...
        try:
            ...
            if novo_sku:
                ...
                if not produto:
                    # Se produto não existe, usa um padrão ou cria (aqui vamos logar erro por enquanto)
                    logger.warning(f"Produto {sku_codigo} não encontrado no cadastro.")
                    return
    ...
    def calcular_metricas_hora(self, equipamento: Equipamento, data_hora: datetime):
        ...
        # Extrai valores iniciais e finais
        contagem_entrada_inicial = dados[0].get('contagem_entrada', 0)
        contagem_entrada_final = dados[-1].get('contagem_entrada', 0)
        contagem_saida_inicial = dados[0].get('contagem_saida', 0)
        contagem_saida_final = dados[-1].get('contagem_saida', 0)
        
        contagem_entrada = int(contagem_entrada_final - contagem_entrada_inicial)
        contagem_saida = int(contagem_saida_final - contagem_saida_inicial)
        
        # Garante não negativo
        contagem_entrada = max(0, contagem_entrada)
        contagem_saida = max(0, contagem_saida)
    ...
    def calcular_metricas_turno(self, equipamento: Equipamento, turno: TurnoProducao, data: datetime):
        ...
        metrica, created = MetricaProducao.objects.update_or_create(
            ...
            defaults={
                ...
                'velocidade_planejada': equipamento.linha.velocidade_planejada,
                'velocidade_real': self._calcular_velocidade_media(dados),
                'tempo_programado': turno.duracao_horas * 60.0,  # em minutos
                'tempo_producao': tempos['tempo_producao'],
                'tempo_parada': tempos['tempo_parada'],
                'tempo_setup': tempos['tempo_setup'],
                'tempo_nao_programado': tempos['tempo_nao_programado'],
