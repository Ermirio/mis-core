...
function genTimestamps(n: number, hoursBack = 8): string[] {
  const now = Date.now();
  const step = (hoursBack * 3600000) / n;
  return Array.from({ length: n }, (_, i) => new Date(now - (n - i) * step).toISOString());
}

function genValues(n: number, mean: number, std: number, seed = 42): number[] {
...
// ---------------------------------------------------------------------------
export function mockAnalyzeTimeseries(variables: { alias: string; tag_influx: string; lsl?: number; usl?: number; nominal?: number }[]) {
  const n = 120;
...
export function mockAnalyzeStats(variables: { alias: string; tag_influx: string; lsl?: number; usl?: number; nominal?: number }[]) {
...
export function mockAnalyzeCorrelation(variables: { alias: string }[]) {
...
export const MOCK_EQUIPMENT_DADOS = (code: string) => ({
  equipamento: code,
...
export const MOCK_REALTIME_ALL = Object.fromEntries(
  ['ENV-01-ENCR', 'ENV-01-TAMP', 'ENV-01-ROT', 'EMP-01-FORM', 'PAL-01-ROBO'].map((code) => [
    code,
    {
      timestamp: new Date().toISOString(),
      status: 'online',
...
// ---------------------------------------------------------------------------
export function mockLinhaStatus(linhaId: string) {
  const makeEquip = (
    sufixo: string, nome: string,
    saida: number, vel: number, descarte: number,
    oee: number, disponibilidade: number, performance: number, qualidade: number,
    temp: number, pressao: number,
  ) => ({
