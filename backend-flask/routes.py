...
_influx_type_conflicts_seen = set()


def coerce_influx_field(name, value):
    if value is None:
        return None
    try:
        ...
        logger.warning("Campo %s ignorado por tipo invalido (%r): %s", name, value, exc)
        return None


def add_dynamic_fields(fields, medicoes, excluded=None):
    excluded = set(excluded or [])
    for name, value in medicoes.items():
        if name in fields or name in excluded:
            continue
        coerced = coerce_influx_field(name, value)
        if coerced is not None:
            fields[name] = coerced


def write_points_resilient(influx_client, points):
    if not influx_client:
        return
    try:
        ...
        for point in points:
            ...
            retry_point['fields'] = {
                name: value
                for name, value in point.get('fields', {}).items()
        ...
        marker = 'input field "'
        if marker in msg:
            conflict_key = msg.split(marker, 1)[1].split('"', 1)[0]
        else:
            escaped_marker = 'input field \\"'
            if escaped_marker in msg:
                conflict_key = msg.split(escaped_marker, 1)[1].split('\\"', 1)[0]
        if conflict_key not in _influx_type_conflicts_seen:
            _influx_type_conflicts_seen.add(conflict_key)
            logger.warning(
                "Conflito de tipo no InfluxDB em %s; regravando apenas campos canonicos. Erro original: %s",
                conflict_key,
                exc,
            )
        else:
            logger.debug("Conflito de tipo repetido no InfluxDB em %s; campos canonicos preservados.", conflict_key)
...
def get_operacao(eq_code):
    try:
        ...
        produzido_turno = float(metrics.get('producao_turno', 0))
        refugo_turno = float(
            metrics.get('refugo_turno')
            or metrics.get('descarte_turno')
            or metrics.get('refugo_turno_acumulado')
            or 0
        )
        ...
        return jsonify({
            ...
            "percentual_descarte_turno": metrics.get('percentual_descarte_turno', 0),
            "toneladas_turno": metrics.get('toneladas_turno', 0),
            "toneladas_refugo_turno": metrics.get('toneladas_refugo_turno', 0),
...
def get_equipamento(eq_code):
    try:
        ...
        return jsonify({
            "equipamento": eq_code,
            "velocidade_atual": int(d.get('last_velocidade_atual', d.get('last_velocidade', 0)) or 0),
            "pecas_produzidas": int(d.get('last_producao_turno_acumulada', d.get('last_contagem_saida', 0)) or 0),
            "refugos": int(
                d.get('last_refugo_turno_acumulado')
                or d.get('last_descarte_turno_acumulado')
                or d.get('last_descarte')
                or 0
            ),
...
def inserir_dados():
    try:
        ...
        if isinstance(d, list):
            ...
            for item in d:
                try:
                    ...
                    slug = item.get('equipamento_slug') or (f"{line}.{eq}" if line and eq else None)
                    area = item.get('area_codigo')
                    factory = item.get('fabrica_codigo')
                    ...
                    if vel_real is not None:
                        ...
                        vel_calc = calc_speed_rpm(identity_key, cont)
                    ...
                    if slug:
                        point_tags["equipment_slug"] = slug
                    if area:
                        point_tags["area"] = area
                    if factory:
                        point_tags["factory"] = factory
                    ...
                    write_points_resilient(influx_client, points)
        ...
        slug = d.get('equipamento_slug') or (f"{line}.{eq}" if line and eq else None)
        area = d.get('area_codigo')
        factory = d.get('fabrica_codigo')
        identity_key = slug or (f"{line}.{eq}" if line else eq)
        ts = d.get('timestamp')
        m = d.get('medicoes', {})
        ...
        add_dynamic_fields(
            fields,
            m,
            excluded=['estado_maquina', 'velocidade_atual', 'ordem_producao', 'sku_codigo', 'planejado_op'],
        )
...
def get_equipamento_dados(codigo):
    ...
    try:
        ...
        query = f"SELECT last(estado_maquina), last(velocidade_atual), last(velocidade), last(contagem_saida), last(producao_turno_acumulada), last(refugo_turno_acumulado), last(descarte_turno_acumulado), last(descarte) FROM production WHERE \"equipment\" = '{codigo}'"
        rs = influx_client.query(query)
        points = list(rs.get_points())
        ...
        if points:
            p = points[0]
            return jsonify({
                "velocidade_atual": float(p.get('last_velocidade_atual') or p.get('last_velocidade') or 0),
                "estado_atual": ESTADOS_MAQUINA.get(int(p.get('last_estado_maquina') or 0), 'Desconhecido'),
                "pecas_produzidas": float(p.get('last_producao_turno_acumulada') or p.get('last_contagem_saida') or 0),
                "refugos": float(
                    p.get('last_refugo_turno_acumulado')
                    or p.get('last_descarte_turno_acumulado')
                    or p.get('last_descarte')
                    or 0
                ),
...
def get_operacao_dados(codigo):
    ...
    try:
        ...
        metrics = state.get('latest_metrics', {}) if state else {}
        produzido_op = metrics.get('producao_op', state.get('producao_op', 0))
        refugo_op = metrics.get('refugo_op_acumulado', state.get('refugo_op_acumulado', 0))
        produzido_turno = metrics.get('producao_turno', state.get('acc_shift', 0))
        refugo_turno = (
            metrics.get('refugo_turno')
            or metrics.get('descarte_turno')
            or metrics.get('refugo_turno_acumulado')
            or state.get('acc_waste_shift', 0)
        )
