...
class ColetorOPC:
    ...
    async def atualizar_configuracao(self) -> bool:
        ...
        try:
            ...
            logger.info(f"✓ Configuração obtida: {total_equipamentos} equipamentos")
            
            # Log detalhado dos equipamentos
            for eq in data.get('equipamentos', []):
                num_tags = len(eq.get('tags_coleta', []))
                logger.info(f"  - {eq['nome']} ({eq['tipo_display']}): {num_tags} tags")
        ...
        try:
            ...
            for url_servidor in servidores:
                try:
                    ...
                    logger.info(f"✓ Conectado a {url_servidor}")
                    
                except Exception as e:
                    logger.error(f"✗ Falha ao conectar a {url_servidor}: {e}")
            ...
            logger.info(f"✓ {len(self.clientes_opc)} conexão(ões) OPC estabelecida(s)")
            return True
    ...
    async def enviar_evento_estado(self, equipamento_codigo: str, estado: str) -> bool:
        ...
        try:
            ...
            logger.info(f"✓ Evento de estado enviado: {equipamento_codigo} → {estado}")
            return True
    ...
    async def coletar_dados_equipamento(self, equipamento: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if 'contagem_entrada' in medicoes and 'contagem_saida' in medicoes:
                ...
                # Calcula descarte
                descarte = contagem_entrada - contagem_saida
        ...
        try:
            ...
            for equipamento in equipamentos:
                ...
                if dados:
                    ...
                    if sucesso:
                        logger.info(f"✓ {dados['equipamento_codigo']}: {len(dados['medicoes'])} medições enviadas")
                    else:
                        logger.warning(f"✗ {dados['equipamento_codigo']}: Falha ao enviar dados")
    ...
    async def finalizar(self):
        ...
        for url, cliente in self.clientes_opc.items():
            try:
                ...
                logger.info(f"✓ Desconectado de {url}")
            except Exception as e:
