...
from app.schemas.analytics import VariableRef
...
class _FakeInflux:
    ...
    async def query_df(self, q: str):
        # Um ponto garante que a rotina avance até a consulta de estado.
        import pandas as pd
        self.queries.append(q)
        return pd.DataFrame(
            {"value": [1.0]},
            index=pd.DatetimeIndex(["2026-04-25T00:00:00Z"]),
        )
...
@pytest.mark.asyncio
async def test_fetch_scopes_reused_equipment_code_to_line():
    inf = _FakeInflux()
    await _fetch_and_aggregate(
        client=inf,
        ...
        line_code="L20",
        tag_influx="velocidade_atual",
        t_start_iso="2026-08-19T00:00:00Z",
        t_end_iso="2026-08-20T00:00:00Z",
        rule="5m",
        off_mask=OffMaskConfig(exclude_states=[]),
    )

    assert len(inf.queries) == 1
    assert '"equipment"=\'E001\'' in inf.queries[0]
    assert '"line"=\'L20\'' in inf.queries[0]


def test_variable_ref_accepts_portuguese_line_field_from_frontend():
    variable = VariableRef.model_validate({
        "tag_influx": "velocidade_atual",
        "equipamento_code": "E001",
        "linha_codigo": "L10",
    })

    assert variable.line_code == "L10"
