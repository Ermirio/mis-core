...
async def _fetch_and_aggregate(
    ...
    equipment_filter = _escape_tag(equipamento_code)
    line_filter = f' AND "line"=\'{_escape_tag(line_code)}\'' if line_code else ""
    q_target = (
        f'SELECT "{field}" AS value FROM "production" '
        f"WHERE \"equipment\"='{equipment_filter}' "
        f"{line_filter} "
